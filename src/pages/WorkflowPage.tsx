...
export function WorkflowPage({
  ...
  const handleDeleteRegisteredAgent = (registeredAgentId: string) => {
    if (!selectedAgent) return;
    const updatedAgents = myAgents.map(agent => {
      if (agent.id === selectedAgent.id) {
        return {
          ...agent,
          registeredAgents: agent.registeredAgents?.filter(ra => ra.id !== registeredAgentId) || []
        };
      }
      return agent;
    });
              ...
              {filteredAgents.length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-4">
                  {filteredAgents.map((agent) => {
                    const toolIds = agent.settings?.tools?.split(",").filter(Boolean) || [];
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <button
                            onClick={() => handleDeleteRegisteredAgent(agent.id)}
                        ...
                        <div className="flex gap-1.5">
                          <button
                            ...
                            className="flex-1 px-2 py-1.5 rounded-md border border-border text-xs hover:bg-muted transition-colors flex items-center justify-center gap-1"
                          >
                          ...
                          <button
                            ...
                            className="flex-1 px-2 py-1.5 rounded-md bg-primary text-primary-foreground text-xs hover:bg-primary/90 transition-colors flex items-center justify-center gap-1"
                          >

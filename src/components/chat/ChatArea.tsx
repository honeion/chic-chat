...
const initialMessages: Message[] = [
  {
    id: "1",
    role: "system",
    content: "AI 어시스턴트와의 대화가 시작되었습니다",
    timestamp: new Date(Date.now() - 60000 * 5),
  },
  {
    id: "2",
    role: "assistant",
    content: "안녕하세요! 저는 AI 어시스턴트입니다. 🤖\n\n무엇을 도와드릴까요? 코드 작성, 문서 분석, 번역 등 다양한 작업을 도와드릴 수 있습니다.",
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-background">
        <div className="text-center space-y-6 animate-fade-in">
          ...
          <div>
            <h2 className="text-2xl font-semibold mb-2">대화를 시작하세요</h2>
            <p className="text-muted-foreground max-w-md">
              왼쪽 사이드바에서 대화를 선택하거나<br />
              새로운 대화를 시작해보세요
            </p>
          </div>
          <div className="flex flex-wrap justify-center gap-3">
            {["AI 어시스턴트", "Workflow 실행", "RAG 검색"].map((label) => (
              <button
                key={label}
...
function getAIResponse(input: string): string {
  const responses: Record<string, string> = {
    "요약해줘": "물론이죠! 요약할 내용을 보내주시면 핵심 내용을 정리해드리겠습니다. 📝",
    "코드 작성": "어떤 언어로 어떤 기능의 코드를 작성해드릴까요? 예를 들어 Python, JavaScript, TypeScript 등 다양한 언어를 지원합니다. 💻",
    "번역해줘": "번역할 텍스트와 목표 언어를 알려주세요. 영어, 한국어, 일본어, 중국어 등 다양한 언어 간 번역이 가능합니다. 🌐",
    "분석해줘": "분석할 데이터나 문서를 공유해주세요. 텍스트 분석, 데이터 분석, 코드 리뷰 등 다양한 분석을 도와드릴 수 있습니다. 📊",
  };
